...
module.exports = function (eleventyConfig) {
	...
	eleventyConfig.addShortcode('respimg', (path, alt, style) => {
		...
		const srcset = eleventyConfig.srcsetWidths
			.map(({ w, v }) => {
				return `${fetchBase}dpr_auto,q_auto,w_${w}/rizalahto.com/${path}.${eleventyConfig.format} ${v}w`
			})
			.join(', ')
	...
	eleventyConfig.cloudinaryCloudName = 'rizalahto'
	...
	let markdownLibrary = markdownIt({
		...
		.use(markdownitlinkatt, {
			pattern: /^(?!(https:\/\/rizalahto\.com|#)).*$/gm,
			attrs: {
				target: '_blank',
				rel: 'noreferrer'
			}
